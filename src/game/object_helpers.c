...
extern u16 gFPS;
#define FPS_MULTIPLIER(x) ((x / 2) * 60.0f / (f32)gFPS)
...
void obj_forward_vel_approach_upward(f32 target, f32 increment) {
    if (o->oForwardVel >= target) {
        ...
        o->oForwardVel += FPS_MULTIPLIER(increment);
    }
}
...
void obj_move_using_vel(void) {
    o->oPosX += FPS_MULTIPLIER(o->oVelX);
    o->oPosY += FPS_MULTIPLIER(o->oVelY);
    o->oPosZ += FPS_MULTIPLIER(o->oVelZ);
}
...
static s32 obj_move_xz(f32 steepSlopeNormalY, s32 careAboutEdgesAndSteepSlopes) {
    ...
    f32 intendedX = o->oPosX + FPS_MULTIPLIER(o->oVelX);
    f32 intendedZ = o->oPosZ + FPS_MULTIPLIER(o->oVelZ);
...
static void obj_move_update_ground_air_flags(UNUSED f32 gravity, f32 bounce) {
    ...
    if (o->oPosY < o->oFloorHeight) {
        ...
        if (o->oVelY < 0.0f) {
            o->oVelY *= FPS_MULTIPLIERF(bounce);
        }
...
static f32 obj_move_y_and_get_water_level(f32 gravity, f32 buoyancy) {
    ...
    o->oVelY += FPS_MULTIPLIERF(gravity + buoyancy);
    if (o->oVelY < -78.0f) {
        o->oVelY = -78.0f;
    }
