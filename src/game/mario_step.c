...
#define FPS_MULTIPLIERF(x) (((x / 2.0f) * 60.0f / (f32)gFPS))
#define FPS_MULTIPLIER(x) (((x / 2.0f) * 60.0f / (f32)gFPS))
...
s32 perform_ground_step(struct MarioState *m) {
    ...
    for (i = 0; i < 4; i++) {
        intendedPos[0] = m->pos[0] + m->floor->normal.y * (FPS_MULTIPLIER(m->vel[0]) / 4.0f);
        intendedPos[2] = m->pos[2] + m->floor->normal.y * (FPS_MULTIPLIER(m->vel[2]) / 4.0f);
...
s32 perform_air_step(struct MarioState *m, u32 stepArg) {
    ...
    for (i = 0; i < 4; i++) {
        intendedPos[0] = m->pos[0] + ((FPS_MULTIPLIER(m->vel[0]))) / 4.0f;
        intendedPos[1] = m->pos[1] + ((FPS_MULTIPLIER(m->vel[1]))) / 4.0f;
        intendedPos[2] = m->pos[2] + ((FPS_MULTIPLIER(m->vel[2]))) / 4.0f;
